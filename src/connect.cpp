...
void initializeCommunication(char* filename, int p)
{
	...
	for (int i = 0; i < NUM_OF_PARTIES; i++)
	{
		...
		addrs[i] = string(ip);
		//cout << addrs[i] << endl;
		ports[2 * i] = 52000 + i*NUM_OF_PARTIES + partyNum;
		ports[2 * i + 1] = 52000 + partyNum*NUM_OF_PARTIES + i;
	}
...
void end_communication(string str)
{
	...
	cout << "Communication, " << str << ", P" << partyNum << ": " 
		 << (double)commObject.getSent()/1000000 << "MB (sent) " 
		 << (double)commObject.getRecv()/1000000 << "MB (recv)" << endl;
